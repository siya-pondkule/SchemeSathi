...
const UserQuizComponent = ({ onClose }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (fetchError && fetchError.code !== 'PGRST116') {
      ...
      alert("Something went wrong while verifying your identity.");
      return;
    }
    ...
    if (existingUser) {
      ...
      if (updateError) {
        ...
        alert("Failed to update your quiz answers.");
      } else {
      ...
      if (insertError) {
        console.error("Error inserting new user data:", insertError.message);
