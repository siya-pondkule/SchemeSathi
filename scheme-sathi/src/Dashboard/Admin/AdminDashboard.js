...
//import { notifyUsersOnNewScheme } from '../../utils/notify';
...
const handleLogout = async () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      ...
      // // Fetch mobile numbers of users from 'users' table
      // const { data: users, error: userError } = await supabase.from('users').select('mobileNumber');
      
      // if (userError) {
      //   console.error('Error fetching user numbers:', userError);
      // } else {
      //   // Extract mobile numbers
      //   const userNumbers = users.map(user => user.mobileNumber);
        
      //   // Notify users via SMS
      //   try {
      //     await notifyUsersOnNewScheme(userNumbers, `New scheme added: ${scheme.schemeName}`); // Send to all users' mobile numbers
      //   } catch (error) {
